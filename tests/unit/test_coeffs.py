...
class TestMie:
    def test_real(self):
        ...
        assert np.all(np.abs(cf.mie(1, [1, 2, 4], [1, 1.3, 3, 2], [1, 2, 1, 1.5], [.1, 0, .2, .1]) - expect) < EPSSQ)
    def test_complex(self):
        ...
        assert np.all(np.abs(cf.mie(3, [1, 2, 3], [1, 2, 3 + 1j, 4], [4, 3 + .1j, 1, 2], [.3, -.1 + .1j, 1 + .1j, .4]) - expect) < EPSSQ)
...
class TestFresnel:
    def test_real(self):
        ...
        assert np.all(np.abs(cf.fresnel(
            [[3, 5], [2, 4]],
    ...
    def test_complex(self):
        ...
        ks = np.stack((np.sqrt(epsilon * mu) - kappa, np.sqrt(epsilon * mu) + kappa), axis=-1)
        kzs = np.sqrt(ks * ks - 1)
...
class TestMieCyl:
    def test_real(self):
        ...
        k0 = 2 * np.pi / 500
        radii = [50, 100]
        assert np.all(np.abs(cf.mie_cyl(kz, m, k0, radii, epsilon, mu, kappa) - expect) / np.max(np.abs(expect)) < 1.1e-2)
    ...
    def test_evanescent(self):
        ...
        k0 = 2 * np.pi / 500
        radii = [30, 100]
        assert np.all(np.abs(cf.mie_cyl(kz, m, k0, radii, epsilon, mu, kappa) - expect) / np.max(np.abs(expect)) < 1e-2)
    ...
    def test_complex(self):
        ...
        m = -1
        epsilon = [4, 2 + 1j, 2 + .02j]
        mu = [1 + 0.5j, 1, 2 + 0.1j]
        kappa = [-0.5, .1 + .1j, -.2 + 0.001j]
        kz = 2 * np.pi / 250
        k0 = 2 * np.pi / 500
        radii = [50, 100]
        assert np.all(np.abs(cf.mie_cyl(kz, m, k0, radii, epsilon, mu, kappa) - expect) / np.max(np.abs(expect)) < 1e-2)
