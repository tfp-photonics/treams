...
def solve(a, b):
    ...
    if not (
        (
            {} in (a.annotations["all"], b.annotations["all"])
            or a.annotations["all"] == b.annotations["all"]
        )
        and (
            a.annotations[-2] == b.annotations[-1 - int(b.ndim != a.ndim - 1)],
        )
    ):
        warnings.warn("incompatible annotations", AnnotatedArrayWarning)

    res.annotations["all"].update(a.annotations["all"])
    res.annotations["all"].update(b.annotations["all"])

    if b.ndim == a.dim - 1:
        for i in range(-1, -1 - b.ndim, -1):
            res.annotations[i - 1].update(a.annotations[i - 1])
            if {} not in (res.annotations[i], b.annotations[i]) and res.annotations[i] != b.annotations[i]:
                warnings.warn("incompatible annotations", AnnotatedArrayWarning)
        ...
        for x in (a, b):
            for i in range(-2, -1 - x.ndim, -1):
                if {} not in (res.annotations[i], x.annotations[i]) and res.annotations[i] != x.annotations[i]:
                    warnings.warn("incompatible annotations", AnnotatedArrayWarning)
...
def lstsq(a, b, rcond="warn"):
    ...
    if not (
        ...
        and (
            a.annotations[0] == b.annotations[0],
        )
    ):
        warnings.warn("incompatible annotations", AnnotatedArrayWarning)

    for i in range(2):
        res[i].annotations["all"].update(a.annotations["all"])
        res[i].annotations["all"].update(b.annotations["all"])
