...
class TMatrix(TMatrixBase):
    ...
    def effective_optical_parameters(self,concentration,units='nm'):
        ...
        index = np.array([[0,size_t,0,size_t],[0,size_t,size_t,2*size_t],[size_t,2*size_t,0,size_t],[size_t,2*size_t,size_t,2*size_t]])
        for i_pol in range(4):
            alphaSI = scaling[i_pol]*T[index[i_pol,0]:index[i_pol,1],index[i_pol,2]:index[i_pol,3]]
            alpha = vector.T.conjugate()@(alphaSI@vector)
            alpha_mean.append(np.sum(np.diag(alpha))/3)
        delta_alpha = alpha_mean[0]*alpha_mean[3]*mu-alpha_mean[1]*alpha_mean[2]*mu
        D = 1-concentration*alpha_mean[0]/(3*epsilon) - concentration*alpha_mean[3]/3 + concentration**2*delta_alpha/(9*mu*epsilon)
        epsilon_eff = 1/eps0*(epsilon+1/D*(concentration*alpha_mean[0]-concentration**2*delta_alpha/(3*mu)))
        mu_eff = 1/mu0*(mu+1/D*(concentration*alpha_mean[3]*mu-concentration**2*delta_alpha/(3*epsilon)))
