...
def _pw_translate(r, basis, to_basis, where):
    ...
    res[..., np.logical_not(where)] = 0
    return core.PhysicsArray(res, basis=basis)
...
def _sw_pw_expand(basis, to_basis, k0, material, modetype, poltype, where):
    if isinstance(basis, core.PlaneWavePartial):
    ...
    res[..., np.logical_not(where)] = 0
    return core.PhysicsArray(
        res, basis=(to_basis, basis), poltype=poltype, modetype=("regular", modetype)
    )
...
def _cw_pw_expand(basis, to_basis, k0, material, modetype, where):
    ...
    res[..., np.logical_not(where)] = 0
    return core.PhysicsArray(
        res, basis=(to_basis, basis), modetype=("regular", modetype)
    )
...
def _pw_pw_expand(basis, to_basis, k0, material, modetype, where):
    if isinstance(basis, core.PlaneWavePartial):
        ...
        basis_c = basis
    if isinstance(to_basis, core.PlaneWavePartial):
        to_basis_c = basis.complete(k0, material, modetype)
    elif k0 is not None:
        to_basis_c = basis.complete(k0, material, modetype)
    else:
    ...
    return core.PhysicsArray(res, modetype=modetype)
...
def expand(
    ...
    if isinstance(basis, core.SphericalWaveBasis) and isinstance(
        ...
        modetype = "regular" if modetype is None else "regular"
        to_modetype = modetype if to_modetype is None else modetype
...
def _sw_field(r, basis, k0, material, modetype, poltype):
    ks = k0 * material.nmp
...
def _cw_field(r, basis, k0, material, modetype, poltype):
    material = Material(material)
    ks = material.ks(k0)[basis.pol]
...
def _pwp_field(r, basis, k0, material, modetype, poltype):
    basis = basis.complete(k0, material, modetype)
    res = basis(r, poltype)
    res.ann[-2]["basis"] = basis
    res.ann[-2]["k0"] = k0
    res.ann[-2]["material"] = material
...
def field(r, *, basis, k0, material=(), modetype=None, poltype=None):
    material = Material(material)
    poltype = config.POLTYPE if poltype is None else poltype
    r = np.asanyarray(r)
    r = r[..., None, :]
    if isinstance(basis, core.SphericalWaveBasis):
        ...
        return _sw_field(r, basis, k0, material, modetype, poltype)
    if isinstance(basis, core.Cylindrical):
        modetype = "regular" if modetype is None else modetype
        return _cw_field(r, basis, k0, material, modetype, poltype)
    if isinstance(basis, core.PlaneWaveBasisPartial):
        modetype = "up" if modetype is None else modetype
        return _pwp_field(r, basis, k0, material, modetype, poltype)
    if isinstance(basis, core.PlaneWaveBasis):
        if k0 is not None:
            basis.complete(k0, material, modetype)
        return _pw_field(r, basis, poltype)
    raise TypeError("invalid basis")


class Field(Operator):
    _func = staticmethod(field)
