...
def effective_optical_parameters(self, concentration, units="nm"):
    ...
    scaling = -6j * np.pi / (c * Z * k_mod ** 3) * np.array([1, 1j * Z, -1j * c, c * Z])
    vector = np.array(
        [
    ...
    for i_pol in range(4):
        alphaSI = (
            scaling[i_pol]
            * T[index[i_pol, 0] : index[i_pol, 1], index[i_pol, 2] : index[i_pol, 3]]
        )
    ...
    epsilon_eff = (
        epsilon
        + (concentration * alpha_mean[0] - concentration ** 2 * delta_alpha / (3 * mu))
        / D
    ) / eps0
    mu_eff = (
        mu
